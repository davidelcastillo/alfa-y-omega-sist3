...
const mapTipoMovimiento = (apiTipo: string): TipoMovimiento => {
  switch (apiTipo) {
    ...
    case 'Egreso por Traspaso':
      return 'Egreso por Traspaso';
...
const fetchMovimientos = async (filters?: FiltersState, depositos?: DepositoDTO[]): Promise<Movimiento[]> => {
  try {
    ...
    // Crear un mapa de nombres de depósito a IDs para resolver los IDs correctos
    const depositoMap = new Map<string, number>();
    if (depositos) {
      depositos.forEach(dep => {
        depositoMap.set(dep.nombre, dep.id);
      });
    }
    ...
    result.items.forEach((item) => {
      if (!movimientosMap.has(item.mov_id)) {
        // Encontrar el ID real del depósito por nombre
        ...
        movimientosMap.set(item.mov_id, {
          ...
          deposito: { id: depositoId, nombre: item.deposito }, // ← Usar el ID real
          productos: [],
        });
      }
...
export default function MovimientosPage() {
  ...
  const reloadMovimientos = async () => {
    ...
    setError(null);
    try {
      const data = await fetchMovimientos(filters, depositos); // ← Pasar depositos
      setMovimientos(data);
    } catch (err) {
    ...
    const loadAllData = async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          // Cargar movimientos después de tener los depósitos
  ...
  const onSubmitModal = async (p: MovimientoPayload) => {
    try {
      // Validación corregida para traspasos
      const isTraspaso = p.tipoMovimiento === 'Egreso por Traspaso' || p.tipoMovimiento === 'Ingreso por Traspaso';
      
      if (isTraspaso) {
        if (!p.depositoOrigenId) throw new Error('Depósito origen requerido');
        if (!p.depositoDestinoId) throw new Error('Depósito destino requerido');
        if (p.depositoOrigenId === p.depositoDestinoId) throw new Error('Los depósitos origen y destino no pueden ser iguales');
      } else {
      ...
      const body: CreateMovimientoBody = {
        depositoId: isTraspaso ? p.depositoOrigenId! : p.depositoId!,
  ...
  return (
    <main className="w-full max-w-none mx-auto px-3 sm:px-4 lg:px-6 py-8 fade-in">
      ...
      <MovimientosFilters
        ...
        tiposMovimiento={tiposMov} // ← Ahora funciona correctamente
        onSearch={onSearch}
        ...
        <AlertDialogContent className="max-w-md">
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  const det = await fetchMovimientoDetalle(created!.id);
                  const movimientoDet: Movimiento = {
                    id: det.movimiento.id,
                    fechaISO: det.movimiento.fecha.slice(0, 10),
                    hora: det.movimiento.hora,
