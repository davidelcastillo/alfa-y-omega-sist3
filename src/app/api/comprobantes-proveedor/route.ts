...
export async function GET(req: Request) {
  try {
    ...
    const sort = searchParams.get('sort');               // -fecha, fecha, total, -total, proveedor, pendiente, -pagado
    ...
    const rows = items.map((cp) => {
      ...
      return {
        ...
        proveedor: { id: cp.proveedor.id, nombre: cp.proveedor.nombre },
        tipo_comprobante: cp.tipoComprobante?.nombre ?? null,
        total,
        pagado,
        pendiente,
        fecha_emision: cp.fecha,
        estado: cp.estado,
      };
    });
