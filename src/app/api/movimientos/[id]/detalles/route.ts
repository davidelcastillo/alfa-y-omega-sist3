...
export async function GET(
  ...
  WITH det AS (
    ...
    JOIN "MovimientoStock" ms
      ON ms.id = dm."movimientoId"
    JOIN "TipoMovimiento" tm
      ON tm.id = ms."tipoMovimientoId"
    JOIN "StockPorDeposito" spd
      ON spd."productoId" = dm."productoId"
     AND spd."depositoId" = ms."depositoId"
    WHERE dm."movimientoId" = ${movId}  -- 👈 parametrizado correctamente
  )
  SELECT
    ...
    (
      ...
      - COALESCE((
          ...
          JOIN "StockPorDeposito" spd2
            ON spd2."productoId" = dm2."productoId"
           AND spd2."depositoId" = m2."depositoId"
          WHERE spd2.id = spd.id
            AND (m2."fecha" > d."fecha"
                 OR (m2."fecha" = d."fecha" AND m2.id > d."mov_id"))
        ), 0)
...
`;
  ...
  const movimientoStr = mov.tipoMovimiento.saldo ? 'Ingreso' : 'Egreso';//creo que va aqui

  // Post-proceso: derivar stockAntes y estado
  const enriched = items.map((it) => {
    const stockDespues = it.stock_despues_calc as number;
    const stockAntes = stockDespues - (it.signo as number) * (it.cantidad as number);
    const estado =
      stockDespues === 0
        ? "AT_ZERO"
  ...
  const totalEgreso = enriched
    ...
    const totalIngreso = enriched.filter(i => i.signo === 1).reduce((s, i) => s + i.cantidad, 0);
    const totalEgreso  = enriched.filter(i => i.signo === -1).reduce((s, i) => s + i.cantidad, 0);
