...
  onClose: () => void
  onSave: (payload: { id?: number; [key: string]: any }) => void
}) {
...
  // MODIFICACIÓN CLAVE: Mapeamos los datos del producto a los estados del formulario
  useEffect(() => {
    if (product) {
      setForm({
...
      // Aseguramos que los IDs seleccionados se precarguen para la edición
      setSel({
...
      });
    } else {
      // Lógica para un producto nuevo (se mantiene igual)
      setForm({
        descripcion: '',
        rubro: '',
        marca: '',
...
  // El useEffect para cargar catálogos se mantiene igual
  useEffect(() => {
    if (!open) return
    let alive = true
    ;(async () => {
...
  // MODIFICACIÓN CLAVE: La función `handleSubmit` ahora solo prepara el payload y llama a `onSave`
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    const payload = {
      // Incluimos el ID si el producto existe
...
    // El padre se encarga de la lógica de guardar
    onSave(payload);
  };
