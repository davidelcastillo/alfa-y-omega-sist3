...
export default function ProductsTable({ products, onEdit, onDelete }: { products: Product[]; onEdit: (id: number) => void; onDelete: (id: number) => void }) {
return (
...
<tr>
{['ID','Descripción','Rubro','Marca','Unidad','Precio Venta','Precio Lista','Estado','Acciones'].map((h) => (
...
<td className="px-6 py-4 text-sm font-semibold text-gray-900">{p.descripcion}</td>
...
<button onClick={() => onEdit(p.id)} className="text-primary-blue hover:text-dark-blue transition-colors p-2 rounded-lg hover:bg-light-pink" aria-label={`Editar ${p.descripcion}`}>
<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
</button>
<button onClick={() => onDelete(p.id)} className="text-red-500 hover:text-red-700 transition-colors p-2 rounded-lg hover:bg-red-50" aria-label={`Eliminar ${p.descripcion}`}>
