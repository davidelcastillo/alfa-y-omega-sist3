...
export default function ComprasModal({
  ...
  // ítem en edición
  const [prodSelId, setProdSelId] = useState<string>("");
  ...
  function addItem() {
    // Usamos prodSelId
    if (!prodSelId) return;
    const prod = productos.find((p) => p.id === prodSelId);
    if (!prod) return;
      ...
      >
        ...
        <div className="flex-auto overflow-y-auto p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* CAMBIO 1: Reemplazar el Select por SearchableSelect para Proveedor */}
            <SearchableSelect
              label="Proveedor *"
              options={proveedores.map(p => ({ id: p.id, name: p.name }))}
              valueId={proveedorId}
              onChange={(opt) => setProveedorId(opt?.id ?? "")}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
              {/* CAMBIO 2: Reemplazar el Select por SearchableSelect para Producto */}
              <SearchableSelect
                label="Producto"
                options={productos.map(p => ({ id: p.id, name: p.name }))}
                valueId={prodSelId}
                onChange={(opt) => {
                  setProdSelId(opt?.id ?? "");
                  // Si el producto tiene un precio, actualizamos el estado de `unit`
                  const productPrice = productos.find(p => p.id === opt?.id)?.price ?? 0;
                  setUnit(productPrice);
                }}
                className="input-focus"
              />
